...
import { handleUpdateOneTopupService } from "@/lib/actions/admin/service-action";
import {
  editTopupServiceSchema,
  type EditTopupServiceFormInput,
  type EditTopupServiceFormValues,
  metaJsonFromRecord,
  toTopupServiceUpdatePayload,
} from "../schema";
...
export default function EditTopupServiceForm({
  service,
  serviceId,
}: {
  ...
  } = useForm<EditTopupServiceFormInput, unknown, EditTopupServiceFormValues>({
    resolver: zodResolver(editTopupServiceSchema),
    defaultValues: {
  ...
  const onSubmit = async (values: EditTopupServiceFormValues) => {
    setFormError(null);

    const mapped = toTopupServiceUpdatePayload(values);
    ...
    startTransition(async () => {
      const response = await handleUpdateOneTopupService(serviceId, mapped.payload!);

      if (!response.success) {
        const message = response.message || "Failed to update topup service";
        setFormError(message);
        toast.error(message);
        return;
      }

      toast.success("Topup service updated successfully");
      router.push(`/admin/services/topup/${serviceId}`);
      router.refresh();
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <LoadingButton
        ...
        loadingText="Updating topup service..."
        className="mt-6"
      >
        Update Mobile Data/Topup
      </LoadingButton>
    </form>
  );
}
