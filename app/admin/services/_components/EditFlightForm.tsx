...
import { handleUpdateOneFlight } from "@/lib/actions/admin/service-action";
import {
  type EditFlightFormInput,
  editFlightSchema,
  type EditFlightFormValues,
  flightClassOptions,
  metaJsonFromRecord,
  toDateTimeLocalValue,
  toFlightUpdatePayload,
} from "../schema";
...
export default function EditFlightForm({
  ...
  const {
  ...
  } = useForm<EditFlightFormInput, unknown, EditFlightFormValues>({
    ...
    defaultValues: {
      ...
      metaText: metaJsonFromRecord(flight.meta),
    },
  });
  ...
  const onSubmit = async (values: EditFlightFormValues) => {
    setFormError(null);

    const mapped = toFlightUpdatePayload(values);
    ...
    startTransition(async () => {
      const response = await handleUpdateOneFlight(flightId, mapped.payload!);

      if (!response.success) {
        const message = response.message || "Failed to update flight";
      ...
      router.push(`/admin/services/flights/${flightId}`);
      router.refresh();
    });
  };
