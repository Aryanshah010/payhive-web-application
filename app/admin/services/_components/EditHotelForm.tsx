...
import { handleUpdateOneHotel } from "@/lib/actions/admin/service-action";
import {
  csvFromArray,
  editHotelSchema,
  type EditHotelFormInput,
  type EditHotelFormValues,
  toHotelUpdatePayload,
} from "../schema";
...
export default function EditHotelForm({
  ...
  const {
  ...
  } = useForm<EditHotelFormInput, unknown, EditHotelFormValues>({
    ...
    defaultValues: {
      ...
      amenitiesText: csvFromArray(hotel.amenities),
      imagesText: csvFromArray(hotel.images),
    },
  });
  ...
  const onSubmit = async (values: EditHotelFormValues) => {
    setFormError(null);

    const payload = toHotelUpdatePayload(values);

    startTransition(async () => {
      const response = await handleUpdateOneHotel(hotelId, payload);

      if (!response.success) {
        const message = response.message || "Failed to update hotel";
      ...
      router.push(`/admin/services/hotels/${hotelId}`);
      router.refresh();
    });
  };
