...
import {
  createInternetServiceSchema,
  type CreateInternetServiceFormInput,
  type CreateInternetServiceFormValues,
  toInternetServiceCreatePayload,
} from "../schema";
import { handleCreateInternetService } from "@/lib/actions/admin/service-action";
...
export default function CreateInternetServiceForm() {
  ...
  const {
    ...
    CreateInternetServiceFormInput,
    unknown,
    CreateInternetServiceFormValues
  >({
    resolver: zodResolver(createInternetServiceSchema),
    defaultValues: {
      provider: "",
      name: "",
      packageLabel: "",
  ...
  const onSubmit = async (values: CreateInternetServiceFormValues) => {
    setFormError(null);

    const mapped = toInternetServiceCreatePayload(values);
    ...
    startTransition(async () => {
      const response = await handleCreateInternetService(mapped.payload!);

      if (!response.success) {
        const message = response.message || "Failed to create internet service";
        setFormError(message);
        toast.error(message);
        return;
      }

      toast.success("Internet service created successfully");
      router.push("/admin/services?tab=internet");
      router.refresh();
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <LoadingButton
        ...
        loadingText="Creating internet service..."
        className="mt-6"
      >
